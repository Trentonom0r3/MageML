...
void Decoder::initialize(const std::string& filePath)
{
    ...
    converter = MageML::Factory::createConverter(torch::kCPU, properties.pixelFormat);
...
bool Decoder::initFilterGraph()
{
    ...
    if (ret < 0)
    {
        CELUX_CRITICAL("Cannot create buffer source: {} ", MageML::errorToString(ret));
        return false;
    }
    ...
    if (ret < 0)
    {
        CELUX_CRITICAL("Cannot create buffer sink: {}", MageML::errorToString(ret));
        return false;
    }
    ...
    if (ret < 0)
    {
        CELUX_ERROR("Error parsing filter graph: {}", MageML::errorToString(ret));
        return false;
    }
    ...
    if (ret < 0)
    {
        CELUX_ERROR("Error configuring filter graph: {}", MageML::errorToString(ret));
        return false;
    }
